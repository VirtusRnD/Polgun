...
import { getLocalizedBlogs, getLocalizedText } from '../constants/blogData'
  ...
  useEffect(() => {
    ...
    async function fetchBlogs() {
      try {
        ...
        console.error('Failed to fetch blogs from CMS:', err)
      } finally {
        if (!cancelled) {
          setLoading(false)
        }
      ...
      <section className="py-24" style={{ backgroundColor: 'var(--th-surface)' }}>
        <div className="max-w-7xl mx-auto px-6 max-w-[var(--layout-max)] lg:px-12">
          {blogs.length === 0 && !loading ? (
            <div className="text-center py-20" style={{ color: 'var(--th-text-muted)' }}>
              <p className="text-base font-semibold">{t('common.no_content', { defaultValue: 'Henüz içerik bulunmamaktadır.' })}</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {blogs.map((blog) => {
                const titleStr = getLocalizedText(blog.title, i18n.language);
                const focusKeywordStr = getLocalizedText(blog.focusKeyword, i18n.language);
                const altTextStr = getLocalizedText(blog.altText, i18n.language);
                const descStr = getLocalizedText(blog.description, i18n.language) ||
                  (Array.isArray(blog.sections) && blog.sections[0]?.paragraphs?.[0] ? getLocalizedText(blog.sections[0].paragraphs[0], i18n.language) : '') ||
                  (Array.isArray(blog.sections) && blog.sections[0]?.heading ? getLocalizedText(blog.sections[0].heading, i18n.language) : '');
